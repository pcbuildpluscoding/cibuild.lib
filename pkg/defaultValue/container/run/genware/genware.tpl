...
import (
  ...
  crg "github.com/pcbuildpluscoding/cibuild/lib/defaultValue/container/run"
...
// NewDVGenVendor
//----------------------------------------------------------------//
func NewDVGenVendor(pkey string) GenwareVendor {
  return func(netAddr string, rw Runware) (Genware, error) {
    ...
    case "ScanVars":
      return crg.NewDVProducer(connex, rw)
    case "Generate":
      writer, err := elm.NewWriter(connex, "Default", rw.String("OutputFile"))
      if err != nil {
        logger.Error(err)
        return nil, err
      } 
      return crg.NewDVComposer(connex, rw, writer)
    default:
